...
const AdminProfile: React.FC = () => {
  ...
  const updateSimpleField = (field: 'name' | 'subtitle', lang: 'en' | 'es' | 'fr', value: string) => {
    setProfile((prev) => ({
      ...prev,
      [field]: {
        ...(prev?.[field] as any),
  ...
  const updateComplexField = (
    field: 'story' | 'why' | 'personality',
    lang: 'en' | 'es' | 'fr',
    subfield: 'title' | 'content',
    value: string
  ) => {
    setProfile((prev) => ({
      ...prev,
  ...
  const updateArrayField = (
    field: 'values' | 'fun_facts',
    lang: 'en' | 'es' | 'fr',
    index: number,
    value: string
  ) => {
    setProfile((prev) => {
      const current = (prev?.[field] as any)?.[lang]?.items || [];
      const updated = [...current];
      updated[index] = value;
      return {
        ...prev,
  ...
  const updateArrayTitle = (field: 'values' | 'fun_facts', lang: 'en' | 'es' | 'fr', value: string) => {
    setProfile((prev) => ({
      ...prev,
      [field]: {
        ...(prev?.[field] as any),
    ...
    <ProtectedRoute>
      <div style={{
        ...
        <div style={{ maxWidth: '900px', margin: '0 auto' }}>
          ...
          {/* Simple Text Fields (name, subtitle) */}
            ...
            <div key={field} style={{ marginBottom: '2rem' }}>
              ...
              {languages.map((lang) => (
                <input
                  ...
                  type="text"
                  value={(profile?.[field] as any)?.[lang.code] || ''}
                  onChange={(e) => updateSimpleField(field, lang.code, e.target.value)}
                  placeholder={`${field} (${lang.label})`}
                  style={{
                    width: '100%',
                    padding: '0.75rem',
          ...
          {/* Complex Fields with title and content */}
          {(['story', 'why', 'personality'] as const).map((field) => (
            <div key={field} style={{ marginBottom: '2rem' }}>
              <label style={{ display: 'block', marginBottom: '0.5rem', textTransform: 'capitalize' }}>
                {field}
              </label>
              {languages.map((lang) => (
                <div key={lang.code} style={{ marginBottom: '1rem' }}>
                  ...
                  <input
                    type="text"
                    value={(profile?.[field] as any)?.[lang.code]?.title || ''}
                    onChange={(e) => updateComplexField(field, lang.code, 'title', e.target.value)}
                    placeholder="Title"
                    style={{
                      width: '100%',
                      padding: '0.75rem',
                  ...
                  />
                  <textarea
                    value={(profile?.[field] as any)?.[lang.code]?.content || ''}
                    onChange={(e) => updateComplexField(field, lang.code, 'content', e.target.value)}
                    placeholder="Content"
                    rows={6}
                    style={{
                      width: '100%',
          ...
          {/* Array Fields with title and items */}
          {(['values', 'fun_facts'] as const).map((field) => (
            <div key={field} style={{ marginBottom: '2rem' }}>
              <label style={{ display: 'block', marginBottom: '0.5rem', textTransform: 'capitalize' }}>
                {field.replace('_', ' ')}
              ...
              {languages.map((lang) => {
                const items = Array.isArray((profile?.[field] as any)?.[lang.code]?.items) 
                ? (profile?.[field] as any)?.[lang.code]?.items
                  : [];
                ...
                return (
                  <div key={lang.code} style={{ marginBottom: '1rem' }}>
                    ...
                    <input
                      type="text"
                      value={(profile?.[field] as any)?.[lang.code]?.title || ''}
                      onChange={(e) => updateArrayTitle(field, lang.code, e.target.value)}
                      placeholder="Title"
                      style={{
                        width: '100%',
                        padding: '0.75rem',
